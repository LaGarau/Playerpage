...
  
  // 🆕 NEW: State for permission denials
...
    }
    
    // 🆕 NEW: Reset locationDenied state before checking
...
        }
      },
      // 🛠️ MODIFIED: Error handler to check for permission denial
      (error) => {
        console.error("Geolocation Error:", error.code, error.message);
        
        // Check for error code 1: PERMISSION_DENIED
...
  }, [qrList, scannedQRIds, mapReady]);
  ...
  // 🛠️ MODIFIED: Wrapped the original startScanner call to handle potential camera denial
  const handleStartScanner = async () => {
    // 🆕 NEW: Reset cameraDenied state
    setCameraDenied(false); 
    
    if (typeof startScanner === 'function') {
      try {
        // Attempt to check camera permission before calling the actual scanner
        // Note: navigator.mediaDevices.getUserMedia is the primary function 
        // that triggers the prompt/NotAllowedError. If your original startScanner
        // function already calls this and handles errors, you might adjust this logic.
        await navigator.mediaDevices.getUserMedia({ video: true, audio: false });

        // If permission check succeeds, call the external startScanner function
        startScanner(); 
      } catch (err) {
        console.error("Camera Access Failed:", err);
        // Check for permission denial errors (like NotAllowedError)
        if (err.name === 'NotAllowedError' || err.name === 'NotReadableError') {
          setCameraDenied(true);
        } else {
          // Fallback alert for other camera issues
          alert("Camera initialization failed. Please try again or check your device settings.");
        }
      }
    }
  };
...
          <div onClick={handleStartScanner} className="flex justify-center items-center w-14 h-14 sm:w-16 sm:h-16 bg-red-600 rounded-full shadow-lg cursor-pointer hover:bg-red-700 transition">
...
      )}
      ...
      {/* 🆕 NEW: Location Denied Popup */}
      {locationDenied && (
        <>
          <div className="fixed inset-0 bg-black/90 z-[1003]" onClick={() => setLocationDenied(false)} />
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-3xl p-10 w-11/12 max-w-md text-center shadow-2xl z-[1004]">
            <img src="/animation/confuse.gif" alt="Location Denied" className="w-32 mx-auto mb-6" />
            <h1 className="text-3xl font-bold text-red-600 mb-4">Location Access Blocked! 🚫</h1>
            <p className="text-lg mb-6 text-black">
              Location is required to play. Please go to your **Device/OS Settings** and **Browser Site Settings** to allow location access for this website.
            </p>
            <button
      ...
      {/* 🆕 NEW: Camera Denied Popup */}
      {cameraDenied && (
        <>
          <div className="fixed inset-0 bg-black/90 z-[1003]" onClick={() => setCameraDenied(false)} />
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-3xl p-10 w-11/12 max-w-md text-center shadow-2xl z-[1004]">
            <img src="/animation/confuse.gif" alt="Camera Denied" className="w-32 mx-auto mb-6" />
            <h1 className="text-3xl font-bold text-red-600 mb-4">Camera Access Blocked! 📷</h1>
            <p className="text-lg mb-6 text-black">
              Camera access is required to scan QR codes. Please go to your **Device/OS Settings** and **Browser Site Settings** to allow camera access.
            </p>
            <button
              onClick={() => { setCameraDenied(false); window.location.reload(); }}
              className="w-full bg-black text-white px-8 py-3 rounded-full text-lg font-bold hover:bg-gray-800 transition"
            >
              I've Updated Settings (Reload)
            </button>
